...
public class MachineMiller extends MachineHeliostatPower
{
	...
	{
		recipeId = CraftingMiller.Instance().GetRecipeId(inventory);
	...
	protected void StartProcessingCycle()
	{
		...
		totalTickCount = 0;
		maxTickCount = CraftingMiller.Instance().GetMaxTick(recipeId);
		result = CraftingMiller.Instance().GetResult(recipeId);
		...
		for (int x = 0; x < GRID_WIDTH; x++)
		{
			for (int y = 0; y < GRID_HEIGHT; y++)
			{
				...
				int count = CraftingMiller.Instance().ComponentsUsedInSlot(
						recipeId, slot);
